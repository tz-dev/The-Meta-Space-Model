...
Description:
    This script performs a neutrino oscillation analysis over sky-binned redshift data.
    It calculates the survival probability P_ee for electron neutrinos using baseline distances 
    inferred from mean redshift values. The script scans over multiple neutrino energies and applies 
    class-specific oscillation parameters (Δm² and θ) if provided.

    Key features:
    - Converts redshift (z) to baseline distance (L) for oscillation modeling
    - Computes P_ee(L, E, Δm², θ) for a range of neutrino energies
    - Applies entropy-based projection to weigh redshift regions
    - Compares hemispheric asymmetry and calculates statistical metrics
    - Appends results to results.csv
    - Generates diagnostic plots (P_ee vs z) with entropy coloring
    - Implements "relaxed" evaluation mode: only one PASS is required for global PASS

Usage:
    python 10b_neutrino_analysis.py [input_csv]
    If no input is provided, defaults to 'z_sky_mean.csv'.

Inputs:
    - CSV file with columns: mean_z, ra_min, dec_min, etc.
    - Configuration from config_external.json (energy list, thresholds, oscillation params)

Outputs:
    - Updated results.csv entries for each energy level
    - Plots saved in ./img/
    - Console summary including pass/fail status

Author: Patch generated by Grok 3 (xAI), modified by MSM
Date: 2025-07-10
Version: 1.0
"""
